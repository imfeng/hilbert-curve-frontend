// Version 1.8.2 hilbert-chart - https://github.com/vasturiano/hilbert-chart
(function (global, factory) {
  ...
  var d3SchemePaired = colors("a6cee31f78b4b2df8a33a02cfb9a99e31a1cfdbf6fff7f00cab2d66a3d9affff99b15928");
  ...
  */
  ...
  var heatmapExports = {};
  var heatmap$1 = {
    get exports(){ return heatmapExports; },
    set exports(v){ heatmapExports = v; },
  };
  ...
  var hilbert = index$1({
    ...
    methods: {
      ...
      focusOn: function focusOn(state, pos, length, transitionDuration) {
        setTimeout(function () {
          ...
          var zoomTransform = transform(state.zoom.__baseElem.node());
          if (!transitionDuration) {
            // no animation
            state.zoom.transform(state.zoom.__baseElem, Object.assign(zoomTransform, destination));
          } else {
            gsapWithCSS.to(zoomTransform, Object.assign({
              duration: transitionDuration / 1000,
              ease: 'power1.inOut',
              onUpdate: function onUpdate() {
                return state.zoom.transform(state.zoom.__baseElem, zoomTransform);
              }
            }, destination));
          }
        });
      ...
      addHeatmap: function addHeatmap(pnts) {
        ...
        var hmElem = d3Select(state.nodeElem).append('div').attr('class', 'hilbert-heatmap').style('top', svgBox.top + state.margin + 'px').style('left', svgBox.left + state.margin + 'px').append('div').style('width', state.canvasWidth + 'px').style('height', state.canvasWidth + 'px');
        heatmap.create({
    ...
    stateInit: function stateInit() {
      return {
        ...
        defaultColorScale: ordinal(d3SchemePaired),
        zoomBox: [[0, 0], [N_TICKS, N_TICKS]],
        axisScaleX: linear().domain([0, N_TICKS]),
        axisScaleY: linear().domain([0, N_TICKS])
      };
    },
    init: function init(el, state, _ref) {
      ...
      var d3El = d3Select(isD3Selection ? el.node() : el);
      ...
      // zoom interaction
      state.zoom = d3Zoom().on('zoom', function (ev) {
      ...
      // Range Tooltip
      var rangeTooltip = d3Select('#range-tooltip');
      if (rangeTooltip.empty()) {
        rangeTooltip = d3Select('body').append('div').attr('id', 'range-tooltip');
      }
      rangeTooltip.classed('hilbert-tooltip', true);
      state.rangeTooltip = rangeTooltip;

      // Value Tooltip
      var valTooltip = d3Select('#val-tooltip');
      if (valTooltip.empty()) {
        valTooltip = d3Select('body').append('div').attr('id', 'val-tooltip');
      }
    ...
    update: function update(state) {
      ...
      var labelAcessor = index(state.rangeLabel);
      var colorAccessor = state.rangeColor ? index(state.rangeColor) : function (d) {
        return state.defaultColorScale(labelAcessor(d));
      };
      var _paddingAccessorFn = index(state.rangePadding);
        ...
        var newPaths = rangePaths.enter().append('g').attr('class', 'hilbert-segment').on('click', function (ev, d) {
          ...
          if (state.showRangeTooltip) {
            ...
            if (state.rangeTooltipContent) {
              state.rangeTooltip.html(index(state.rangeTooltipContent)(d));
            } else {
              // default tooltip
              var rangeLabel = index(state.rangeLabel);
        ...
        var _loop = function _loop() {
          ...
          if (d.pathVertices.length === 0) {
            ...
            var _d$startCell$map = d.startCell.map(function (c) {
              ...
              _d$startCell$map2 = _slicedToArray$1(_d$startCell$map, 2),
            ...
            var _d$startCell$map3 = d.startCell.map(function (c) {
              ...
              _d$startCell$map4 = _slicedToArray$1(_d$startCell$map3, 2),
...
//# sourceMappingURL=hilbert-chart.js.map
